 ...
 * @author Marmota
 */
public class StrategyStrongest extends Strategy{
    ...
    public StrategyStrongest(int agNumber, int[][] matrix, int argStart, int argAmount, int[] importance) {
    ...
    public int firstArgument() {
        ...
        for(int i = ownArgStart;i<(ownArgStart + argAmount/2);i++){//para cada argumento disponível
            for(int j = 1; j<argStart;j++ ){//para cada relação com um objetivo
                if(Math.abs(matrix[i][j]) > strength){
                    strength = Math.abs(matrix[i][j]);
                    strongest = i;
                }
            }
        }
    ...
    public void counterChoices(int arg) {
        ...
        for(int i = 0;i<counterChoices.length;i++){// para cada argumento em choices
           ...
           strength = 0;
           for(int j = 1; j<argStart;j++){//pega a maior força
               if(Math.abs(matrix[counterChoices[i]][j]) > strength){
                   strength = Math.abs(matrix[counterChoices[i]][j]) * importance[j-1];
               }
           }
